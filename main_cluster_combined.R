...
rscw_m_AR_train <- read.csv("rscw_m3_AR2_c2_train.csv")
rscw_m_AR_test <- read.csv("rscw_m3_AR2_c2_test.csv")
...
for (clu in 1:2){
  ## ------ create training and test dataset for each cluster ------
  ...
  final.result <- rbind(final.result, reg.result)
}

## ------ Performance Evaluation ------
error <- RMSE(final.result$actual,final.result$prediction)
cat('\n')
cat('RMSE:',error,'\n')

ceiling_err <- RMSE(final.result$actual,ceiling(final.result$prediction))
cat('\n')
cat('RMSE after ceiling:',ceiling_err,'\n')

floor_err <- RMSE(final.result$actual,floor(final.result$prediction))
cat('\n')
cat('RMSE after flooring:',floor_err,'\n')

round_err <- RMSE(final.result$actual,round(final.result$prediction))
cat('\n')
cat('RMSE after rounding:',round_err,'\n')

## ------ fitting result visualiztion ------
library(reshape2)
cmp.m <- melt(final.result[,c("Hour","actual","prediction")], 
              id.vars = "Hour")
